...
const Unsubscribe = () => {
  ...
  const fetchRepositories = async () => {
    try {
      const response = await axios.get(
        `https://localhost:3000/subscriptions/${email}`
      );
  ...
  const handleUnsubscribe = async (repoId) => {
    try {
      await axios.post(`https://localhost:3000/subscriptions/unsubscribe`);
        ...
        {repositories.length === 0 ? (
          ...
          <div className="w-full flex flex-col items-center justify-center gap-4">
            {repositories.map((repo) => (
              <div
                key={repo.id}
                className="bg-white bg-opacity-75 p-6 rounded-lg shadow-lg w-full max-w-md flex justify-between items-center"
              >
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg shadow-lg w-80">
              ...
              <div className="flex justify-between gap-4 mt-6">
                ...
                <button
                  ...
                  onClick={() =>
                    confirmModal.repoId &&
                    handleUnsubscribe(confirmModal.repoId)
                  }
                >
                  Confirm
                </button>
