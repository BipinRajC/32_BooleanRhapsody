...
const Unsubscribe = () => {
  ...
  const fetchRepositories = async () => {
    try {
      const response = await axios.post(
        `${BASE_URL}/subscription/getRepositoriesAndBranchOfUser`,
        {
          email: email,
        }
      );
  ...
  const handleUnsubscribe = async (repoId) => {
    try {
      ...
      const { repository, branch } = selectedRepo;
      ...
      const response = await axios.post(
        `${BASE_URL}/subscription/deleteSubscription`,
        {
          email: email,
          repository,
          branch,
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {repositories.map((repo) => (
              <div
                key={repo.id}
                className="p-4 border rounded-lg shadow-md flex justify-between items-center bg-gray-100"
              >
                <div>
                ...
                <button
                  ...
                  onClick={() =>
                    setConfirmModal({ show: true, repoId: repo.id })
                  }
                >
                  <Trash2 size={16} />
                  Unsubscribe
          ...
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg shadow-lg w-80">
            ...
            <div className="flex justify-between gap-4 mt-6">
              ...
              <button
                ...
                onClick={() =>
                  confirmModal.repoId && handleUnsubscribe(confirmModal.repoId)
                }
              >
                Confirm
              </button>
